...
Here is an example of the heat map in rviz2 with signal db's displayed as text:

<img width="2667" height="1533" alt="heatmap_rviz" src="https://github.com/user-attachments/assets/65f45f8f-e32b-4d28-82d1-ca86eed9b076" />
...
```
You need to have your robot running and publishing the following topics:
- /global_costmap/costmap
- /odometry/local
- /gps/filtered   (if *use_gps* parameter is set)

To run either node separately or together use the following commands:
```
cd ~/robot_ws
source install/setup.bash
# The following runs all three nodes - logger, visualizer and heat mapper:
...
If you have GUI (Desktop) on your robot, or copied the database to your development workstation, run heat mapper standalone:

ros2 launch wifi_logger_visualizer heat_mapper.launch.py standalone:=true

```
**Tips:**
- in Rviz, look for topics starting with "wifi". See screenshot above.
- you can change parameters in [wifi_logger_config.yaml](https://github.com/slgrobotics/wifi_logger_visualizer/blob/main/config/wifi_logger_config.yaml)

**Note:** 
_sqlite3_ database file will be created in the directory where you run Logger node: 
```
...
This is how the heat map looks in *standalone* mode:
